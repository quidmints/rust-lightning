...
mod tests {
	...
	fn test_anchor_descriptor_previous_utxo_script_pubkey_uses_p2wsh() {
		...
		let anchor_descriptor = AnchorDescriptor {
			...
			value: Amount::from_sat(ANCHOR_OUTPUT_VALUE_SATOSHI),
			// QU!D PATCH (M9g): `per_commitment_point` is our field, added for SIMPLE-TAPROOT
			// anchors. This upstream test pins the P2WSH (keyed-anchor) path explicitly, which is
			// the non-taproot case — and `taproot_anchor_spk` returns `None` for exactly that — so
			// `None` is the value the test means, not merely one that compiles.
			per_commitment_point: None,
		};
